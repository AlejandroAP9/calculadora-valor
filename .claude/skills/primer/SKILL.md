...
# Primer: Contexto Raíz

Este proyecto fue creado con **Raíz**, una template optimizada para desarrollo Agent-First. Al ejecutar este skill, el agente entiende inmediatamente que tiene disponible y como trabajar.

## Lo Que Ya Sabes (Raíz DNA)

### Sistema de Raíces (Stack Fijo)
...
### Skills Disponibles
Activa el skill adecuado según la tarea:

| Skill | Responsabilidad |
|-------|-----------------|
| `add-login` / `add-payments` / `add-emails` / `add-mobile` | Auth, pagos, emails, PWA |
| `prp` + `bucle-agentico` | Planificar e implementar features complejas |
| `ai` | Capacidades de IA (chat, RAG, vision, tools) |
| `supabase` | Tablas, RLS, migraciones, queries |
| `playwright-cli` | Testing automatizado |
| `website-3d` | Landing cinematica |
| `image-generation` / `video-visuals` | Imágenes y visuales |
...
- `website-3d` - Crear landing page de alta conversion
- `add-login` - Inyectar sistema de autenticacion completo
- `eject-raiz` - Eliminar configuracion Raíz
- `update-raiz` - Actualizar a la ultima version
...
- Tu traduces a codigo usando el Sistema de Raíces
