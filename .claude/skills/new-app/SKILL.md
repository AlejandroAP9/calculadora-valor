...
### Capacidades detectadas → skill que las construye
A partir del flujo y los datos, marca qué capacidades necesita y con qué skill:
- Login / cuentas → `add-login` · Cobro / suscripción → `add-payments`
- Emails → `add-emails` · PWA / push → `add-mobile`
- Subir / ver archivos → `add-storage` · Trabajo programado → `add-cron`
- Avisos al usuario → `add-notifications` · Chat / IA / RAG → `ai`
- Landing → `raiz-landing`
Lo que calza en una skill NO se reinventa a mano.
